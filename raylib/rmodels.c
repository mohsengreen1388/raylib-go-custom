...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileText != NULL)
    {
        ...
        if (ret != TINYOBJ_SUCCESS)
            ...
            TRACELOG(LOG_INFO, "MODEL: [%s] OBJ data loaded successfully: %i meshes/%i materials", fileName, meshCount, materialCount);
    ...
    if (result == cgltf_result_success)
    {
        ...
        for (unsigned int i = 0, meshIndex = 0; i < data->meshes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                if (data->meshes[i].primitives[p].type != cgltf_primitive_type_triangles)
                    continue;
...
{
    if (frameCount >= anim.frameCount)
    {
        frameCount = 0;
    }
    return anim.framePoses[frameCount][boneId];
}
